...
if "sk_test" in STRIPE_SECRET_KEY and not DJANGO_DEBUG and not STRIPE_TEST_OVERRIDE:
    raise ValueError("invalid  stripe key for production deployment")
...
def get_subscription(sub_stripe_id, raw=True):    
...
def get_customer_active_subscriptions(customer_stripe_id, raw=True):    
...
def cancel_subscription(sub_stripe_id, reason="", feedback="other",cancel_at_period_end=False, raw=True):    
    if cancel_at_period_end:
        response=stripe.Subscription.modify(        
            ...
            #     "feedback" : feedback
            # },
            cancel_at_period_end=cancel_at_period_end, # check stripeAPI docs. think I should modify the susbscription
        )
    else:
